...
def forward(input, target, input_last, input_prev, models, losses):
    ...
    # pred frame and motion
    pred_frame, pred_motion = generator(input)
    # ===== Motion Ground Truth (frame difference) =====
    motion_gt = target - input_last    # frame5 - frame4
    ...
    # base generator loss
    # base generator loss
    loss_gen = coefs[0]*inte_l + coefs[1]*grad_l + coefs[2]*adv_l + coefs[3]*flow_l

    # ===== Motion Head Loss =====
    lambda_motion = 0.1
    motion_l = losses['motion_loss'](pred_motion, motion_gt)
