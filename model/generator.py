...
class UNet(nn.Module):
    def __init__(self, input_channels, output_channel=3):
        ...
        self.frame_head = nn.Conv2d(64, output_channel, kernel_size=3, padding=1)

        # Motion prediction head (frame-difference)
        self.motion_head = nn.Conv2d(64, output_channel, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        pred_frame = self.frame_head(x)
        pred_motion = self.motion_head(x)

        pred_frame = torch.tanh(pred_frame)
        pred_motion = torch.tanh(pred_motion)
...
if __name__=="__main__":
    ...
    print('frame:', pred_frame.shape)
    print('motion:', pred_motion.shape)
